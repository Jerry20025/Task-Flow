...
export const sendTicketAssignedEmail = async (to: string, ticketName: string, projectKey: string, ticketId: string) => {
    const subject = `🎫 You have been assigned a new ticket: ${projectKey}-${ticketId}`;
    const url = `${config.clientUrl}/project/${projectKey}/ticket/${ticketId}`;
    ...
    await sendEmail(to, subject, "", baseHtml(content)).catch(console.error);
};

export const sendTicketUpdatedEmail = async (toEmails: string[], ticketName: string, projectKey: string, ticketId: string, updateSummary: string) => {
    ...
    for (const email of toEmails) {
        if (email) await sendEmail(email, subject, "", baseHtml(content)).catch(console.error);
    }
};
